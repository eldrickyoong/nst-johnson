...
if __name__ == "__main__":
    ...
    style_path = "style_the-western-railway-at-its-exit-from-paris_datapoints_160000_cw_1.0_sw_400000.0_tw_0.pth"
    parser.add_argument("--content_input", type=str, help="Content image(s) to stylize", default='figures.jpg')
    parser.add_argument("--batch_size", type=int, help="Batch size used only if you set content_input to a directory", default=5)
    parser.add_argument("--img_width", type=int, help="Resize content image to this width", default=550)
    parser.add_argument("--model_name", type=str, help="Model binary to use for stylization", default=style_path)
    ...
    style_path_format = lambda batch, epoch: f"ckpt_style_red-cables-1940_cw_1.0_sw_400000.0_tw_0_epoch_0_batch_{batch}.pth"
    epochs = [0]
    batches = [1999, 5999, 11999, 19999]
    ...
    for epoch in epochs:
        for batch in batches:
            ...
            style_binaries_path = style_path_format(batch, epoch)
            inference_config['model_name'] = style_binaries_path
